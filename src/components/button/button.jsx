...
const Button = ({ title, icon, variant, isLoading, isDisabled, onPress }) => {
  ...
  return (
    <View style={styles.container}>
      <TouchableContainer underlayColor={underlayColor} disabled={isInactive} onPress={onPress}>
        <View style={[styles.button, { padding, backgroundColor, opacity: isInactive ? 0.6 : 1 }]}>
          {isLoading && <Spinner size={22} color={color} />}
          {showIcon && <Icon name={icon} size={22} color={color} />}
          <Text style={[styles.title, { color, marginLeft: titleMarginLeft }]}>{title}</Text>
        </View>
      </TouchableContainer>
    </View>
  );
